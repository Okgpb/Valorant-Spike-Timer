...
RESOLUTION_SETTINGS = {
    ...
    '2.5k': {
        'description': '2.5K (2560x1600)',
        'roi': {'left': 1220, 'top': 25, 'width': 120, 'height': 110},
        'b64_data': IMAGE_DATA['2.5k']
    }
}
...
class ScoreDisplay(OverlayWindow):
    ...
    def __init__(self, master):
        super().__init__(master, "+1600+50") # Adjust position based on your screen
...
def main():
    ...
    root = tk.Tk()
    root.title("Val_Timer by Okgpb")
    root.geometry("450x250")
    root.minsize(450, 250)
    root.resizable(False, False)

    # --- UI Components ---
    title_label = tk.Label(root, text="Select your screen resolution", font=("Arial", 14))
    btn_frame = tk.Frame(root)
    buttons = {
        '1k':   tk.Button(btn_frame, text="1920x1080", width=12),
        '2k':   tk.Button(btn_frame, text="2560x1440", width=12),
        '2.5k': tk.Button(btn_frame, text="2560x1600", width=12)
    }
    debug_mode_var = tk.BooleanVar()
    debug_check = tk.Checkbutton(root, text="Enable Debug Mode", variable=debug_mode_var)
    status_label = tk.Label(root, text="Status: Not Started", font=("Arial", 12), fg="grey", width=35)
    stop_button = tk.Button(root, text="Stop Timer", state="disabled", width=15)
    
    # --- Layout ---
    root.columnconfigure(0, weight=1)
    title_label.grid(row=0, column=0, pady=(15, 5))
    btn_frame.grid(row=1, column=0, pady=5)
    buttons['1k'].pack(side="left", padx=5)
    buttons['2k'].pack(side="left", padx=5)
    buttons['2.5k'].pack(side="left", padx=5)
    debug_check.grid(row=2, column=0, pady=5)
    status_label.grid(row=3, column=0, pady=5)
    stop_button.grid(row=4, column=0, pady=3)
    ...
    def start_detector(selection):
        ...
        res_map = {'1k': (1920, 1080), '2k': (2560, 1440), '2.5k': (2560, 1600)}
        ...
        status_label.config(text="Status: Running...", fg="#2ed573")
    ...
    def on_closing():
        # Ensure the background thread also exits when closing the window
        if detector_thread and detector_thread.is_alive():
            print("Window closing, stopping background thread...")
            stop_event.set()
        root.destroy()
